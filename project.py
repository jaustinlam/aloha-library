...
import secret_key
# SQLite methods
import categories_methods
import books_methods
import login_methods
...
def fbconnect():
    ...
    output += ' " class="circle" style = "display: block; margin: auto; width: 200px; height: 200px;"> '
...
def showCategories():
    ...
    '''
    categories = categories_methods.categoriesQuery()
    return render_template('categories.html', categories = categories, session = session)
...
def editCategory(categories_id):
    ...
    if request.method == 'POST':
        name = request.form['name']
        if user:
            if user.id == category.user_id or user.administrator == True: # Checks to see if user is author of category
                ...
                print "user is not author of the category"
                return render_template('editcategories.html', category = category)
        else:
...
def delCategory(categories_id):
    ...
    if request.method == 'POST':# Need to implement auth to delete
        if user.id == category.user_id or user.administrator == True: # Checks to see if user is author of category or admin
            categories_methods.delCategory(category)
            flash("Successfully deleted %s" % name)
            return redirect(url_for('showCategories'))
        elif user:
...
@app.route('/categories/<int:categories_id>/books/new', methods = ['GET', 'POST'])
def newBook(categories_id):
    ...
    if request.method == 'POST':
        if user:
            ...
            user_id = user.id
            books_methods.newBook(category.id, title, isbn, url, author, description, user_id)
            flash("Successfully created new Book %s" % title)
            return redirect(url_for('showBooks', categories_id = category.id))
        else:
...
@app.route('/categories/<int:categories_id>/books/<int:book_id>/edit', methods = ['GET', 'POST'])
def editBook(categories_id, book_id):
    ...
    user = login_methods.getUserBySession(session)
    if request.method == "POST":
        if user.id == book.user_id or user.administrator == True:
            ...
            return redirect(url_for('showBooks', categories_id = categories_id, book_id = book_id))
        elif user:
...
@app.route('/categories/<int:categories_id>/books/<int:book_id>/delete', methods = ['GET', 'POST'])
...
@app.route('/categories/<int:categories_id>/books/<int:book_id>/checkout', methods = ['GET','POST'])
def checkOutBook(categories_id, book_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('checkoutbooks.html', categories_id = categories_id, book = book)

# Check in a Book.
@app.route('/categories/<int:categories_id>/books/<int:book_id>/checkin', methods = ['GET', 'POST'])
def checkInBook(categories_id, book_id):
    ...
    if request.method == 'POST':
        if user.id == lastcheckout.user_id or user.administrator == True:
            books_methods.checkInBook(book, user.id)
            flash("Successfully checked in %s" % book.title)
            return redirect(url_for('showBooks', categories_id = categories_id))
        elif user:
            flash("Sorry chap you can't check this in")
            return render_template('checkinbooks.html', categories_id = categories_id, book = book)
        else:
...
def userPage(user_id):
    ...
    if request.method == 'POST':
        if editor.id == int(session['facebook_id']) or editor.administrator == True:
            ...
            if ischecked != user.administrator and editor.administrator == True:
        ...
        if user.id == int(session['facebook_id']) or editor.administrator == True:
